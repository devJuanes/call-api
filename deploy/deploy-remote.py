#!/usr/bin/env python3
"""Despliega MatuCall API en el VPS (call.api.matubyte.com) → /root/apps/call-api."""
...
SITE = "https://call.api.matubyte.com"
DOMAIN = "call.api.matubyte.com"
...
def production_env(local: dict[str, str]) -> str:
    return "\n".join(
        [
            "# MatuCall API — producción (call.api.matubyte.com)",
            "MATUDB_DEMO=false",
            "PORT=4110",
            "NODE_ENV=production",
            "CORS_ORIGIN=*",
...
def main() -> int:
    ...
    run(
        ...
        "rm -f /etc/nginx/sites-enabled/call.api.matudb.com /etc/nginx/sites-available/call.api.matudb.com /etc/nginx/sites-enabled/call-api.matudb.com /etc/nginx/sites-available/call-api.matudb.com || true\n",
    )
